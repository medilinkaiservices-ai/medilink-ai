...
import { getStoredAccount, registerRole } from "../utils/session";
...
function SellerRegistration({ sellerId, onRegistered }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      const currentUid = auth.currentUser?.uid || sellerId || "guest";
      ...
      registerRole("seller", {
        ...
        shopId: sellerRef.id,
        ownerUid: currentUid,
        uid: account?.uid || currentUid,
        authProvider: account?.authProvider || "local",
        otpVerified: account?.otpVerified || false
      });
      ...
      <div className="auth-layout">
        ...
        <form className="auth-card" onSubmit={handleSubmit}>
          ...
          <input
            ...
            disabled={Boolean(account?.otpVerified)}
            required
          />
